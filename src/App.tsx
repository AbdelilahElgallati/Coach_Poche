...
export default function App() {
    ...
    const handleGeneratePlan = async (userProfile: UserProfile) => {
        ...
        try {
            const data = await generateWorkoutPlan(userProfile);
            setPlan(data);
        } catch (err: any) {
    ...
    const adjustPlan = async (feedback: string) => {
        ...
        setLoading(true);
        try {
            const data = await generateWorkoutPlan(profile, plan, feedback);
            setPlan(data);
        } catch (err) {
    ...
    return (
        <main className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center max-w-7xl mx-auto">
            ...
            {!plan ? (
                <ProfileForm onSubmit={handleGeneratePlan} isLoading={loading} initialData={profile} />
            ) : (
                <div className="flex flex-col items-center w-full animate-in fade-in zoom-in-95 duration-500">
                    <div className="w-full max-w-4xl flex justify-start mb-6">
                        <button
                            ...
                            className="group flex items-center gap-2 text-gray-300 hover:text-indigo-400 font-medium transition-colors py-2 px-4 rounded-lg hover:bg-white/10"
                        >
            ...
            <footer className="mt-16 text-center text-sm text-gray-500">
                <p>© {new Date().getFullYear()} Coach de Poche. Propulsé par Groq AI.</p>
            </footer>
        </main>
    );
}
